import { NextRequest, NextResponse } from 'next/server';
import { getMCPClientManager } from '@/lib/mcp-client';
import type { GetPromptRequest, MCPApiResponse, PromptResult } from '@/lib/mcp-types';

export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as GetPromptRequest;

    if (!body.serverId) {
      return NextResponse.json<MCPApiResponse<null>>(
        { success: false, error: 'Server ID is required' },
        { status: 400 }
      );
    }

    if (!body.promptName) {
      return NextResponse.json<MCPApiResponse<null>>(
        { success: false, error: 'Prompt name is required' },
        { status: 400 }
      );
    }

    const manager = getMCPClientManager();
    const result = await manager.getPrompt(body.serverId, body.promptName, body.arguments);

    return NextResponse.json<MCPApiResponse<PromptResult>>({
      success: true,
      data: result,
    });
  } catch (error) {
    console.error('MCP get prompt error:', error);
    return NextResponse.json<MCPApiResponse<null>>(
      { success: false, error: error instanceof Error ? error.message : 'Failed to get prompt' },
      { status: 500 }
    );
  }
}
